...
## 2025-06-01 - Avoid Eager Array Allocation and Full Traversal in SwiftUI Computed Properties
**Learning:** Checking `!array.filter { ... }.isEmpty` creates an entire filtered array just to see if any items match, resulting in unnecessary memory allocation and O(N) evaluation. Similarly, `reduce` on an eagerly evaluated computed property (like `selectedResults.reduce`) introduces intermediate arrays during each UI cycle.
**Action:** Always use `.contains(where:)` to short-circuit boolean existence checks. Chain `.lazy.filter` before `.reduce` to avoid intermediate array allocations in computed properties.
