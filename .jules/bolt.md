...
## 2024-05-30 - Lazy Filter Evaluation
**Learning:** Eager `.filter` operations create unnecessary intermediate arrays, causing memory churn and overhead on every view update. In SwiftUI, where view properties are re-evaluated frequently, this can impact performance.
**Action:** Chain `.lazy` before `.filter` when immediately followed by operations like `.count` or `.sorted()` to prevent allocations of intermediate arrays and reduce memory churn.
