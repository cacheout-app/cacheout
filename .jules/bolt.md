...
## 2024-05-29 - Avoid array allocation for emptiness checks
**Learning:** Checking if a filtered collection has elements by using `!array.filter { ... }.isEmpty` or relying on computed properties that return a filtered array (like `selectedResults`) causes unnecessary intermediate array allocations, memory churn, and O(N) traversals on every UI render in SwiftUI.
**Action:** Use `.contains(where:)` to short-circuit evaluation and prevent array allocations when checking for the existence of elements matching a condition.
