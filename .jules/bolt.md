...
## 2025-10-25 - Avoid eager filtering for emptiness checks
**Learning:** Using `array.filter { condition }.isEmpty` forces an eager array allocation containing all matched elements, just to check if it's empty.
**Action:** To optimize Swift collection processing and avoid memory churn (especially in SwiftUI computed properties), use `.contains(where: condition)` instead of checking `.isEmpty` on a filtered array. Also, use `.lazy.filter` to avoid array allocations when counting or reducing.
