...
## 2024-05-24 - Optimize Collection Processing
**Learning:** `lazy.filter` shouldn't be chained into `count` or `isEmpty` without consideration, but instead of using `isEmpty` on `lazy.filter`, we can use `contains(where:)` to short circuit the execution to prevent evaluating all elements. Also for the `reduce` chaining after `lazy.filter`, we can chain `lazy.filter` before `.reduce` to avoid intermediate array allocations.
**Action:** Chain `.lazy.filter` before `.reduce` or use `contains(where:)` instead of `isEmpty` checks.
