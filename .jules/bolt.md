...
## 2026-03-20 - Avoid Eager Filters in Computed Properties
**Learning:** In SwiftUI view models, computed properties are evaluated frequently. Eager `.filter` operations before `.reduce`, `.count`, or `.isEmpty` create unnecessary intermediate arrays, causing memory churn and performance degradation.
**Action:** Chain `.lazy.filter` before terminal operations like `.reduce` or `.count`. Replace `.filter { ... }.isEmpty` with `.contains(where:)` to enable short-circuit evaluation without memory allocation.
