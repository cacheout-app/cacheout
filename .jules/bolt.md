...
## 2024-06-07 - Optimize computed property short-circuiting
**Learning:** Checking `.isEmpty` on a filtered array or evaluating a full `reduce` operation just to check for existence causes unnecessary memory churn and O(N) evaluation in SwiftUI computed properties.
**Action:** Chain `.lazy.filter` before `.reduce` to avoid array allocations, and use `.contains(where:)` to short-circuit existence checks instead of using `.isEmpty` on a `.filter` result.
