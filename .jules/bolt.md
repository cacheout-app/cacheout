...
## 2025-10-25 - Lazy Evaluation in Computed Properties
**Learning:** Checking if an eagerly filtered array is empty (e.g., `selectedResults.isEmpty` where `selectedResults` is `scanResults.filter { $0.isSelected }`) allocates an intermediate array unnecessarily. This causes memory churn, particularly in SwiftUI computed properties evaluated on every render loop.
**Action:** Use `.contains(where: \.isSelected)` to short-circuit evaluation instead of an eager `.filter {}.isEmpty` check. Furthermore, avoid creating intermediate arrays for `.reduce` operations by chaining `.lazy.filter { ... }.reduce(...)`.
