...
## 2024-05-06 - Avoid Eager Array Filtering
**Learning:** Calling `.filter` directly before `.count` or `.sorted` forces the creation of an intermediate `Array`, increasing memory churn and allocation overhead.
**Action:** Chain `.lazy.filter` before `.count` or `.sorted` to prevent intermediate array allocations.
