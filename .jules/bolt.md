...
## 2024-05-29 - Short-circuiting and Lazy Filtering in SwiftUI Computed Properties
**Learning:** Checking `.isEmpty` on a property that eager-filters an array (e.g., `!items.filter { ... }.isEmpty`) forces O(N) evaluation and allocates an intermediate array every time it's accessed, which is deadly in SwiftUI computed properties.
**Action:** Always use `.contains(where:)` instead of `.filter { ... }.isEmpty` to short-circuit the evaluation. For reductions or counts, use `.lazy.filter` to skip intermediate allocations.
