...
## 2026-04-05 - Lazy Filter Chains in SwiftUI Computed Properties
**Learning:** Eager `.filter` operations in SwiftUI computed properties execute on every view re-render, allocating temporary intermediate arrays that cause unnecessary memory churn.
**Action:** Chain `.lazy.filter` before terminal operations like `.count` or `.sorted()` in UI components to avoid intermediate array allocations during rapid state updates.
