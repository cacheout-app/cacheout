...
## 2024-05-24 - Avoiding intermediate array allocations in SwiftUI computed properties
**Learning:** SwiftUI computed properties (like `hasSelection`) are evaluated very frequently during render cycles. Using eager `.filter { ... }.isEmpty` creates intermediate arrays and causes unnecessary O(N) memory churn on every layout pass.
**Action:** Chain `.lazy.filter` before `.reduce` or `.count` to prevent allocations, and use `.contains(where:)` to short-circuit boolean checks instead of checking `.isEmpty` on filtered arrays.
