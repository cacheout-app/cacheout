...
## 2024-05-30 - Short-circuiting Computed Property Evaluations in SwiftUI
**Learning:** In SwiftUI, computed properties on `@Observable` or `ObservableObject` view models are often re-evaluated on every render cycle (e.g., when bound to `.disabled()`). Using `.filter { ... }.isEmpty` inside these properties eagerly allocates a new array and takes O(N) time on every render frame.
**Action:** Always use `.contains(where:)` instead of `.filter { ... }.isEmpty` to short-circuit the evaluation, enabling O(1) best-case performance and eliminating memory churn.
