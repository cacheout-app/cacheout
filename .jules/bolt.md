...
## 2024-05-26 - Optimize SwiftUI Computed Properties
**Learning:** In SwiftUI, computed properties are evaluated frequently during re-renders. Using eager `.filter` operations followed by `.isEmpty` or `.reduce` causes unnecessary O(N) array allocations, which leads to memory churn and frame drops.
**Action:** Always chain `.lazy.filter` before `.reduce` to avoid intermediate array allocations, and use `.contains(where:)` instead of checking `.isEmpty` on a filtered array to short-circuit evaluation and save O(N) iterations.
