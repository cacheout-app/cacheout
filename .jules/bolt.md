...
## 2024-05-30 - Eager Collection Filtering in SwiftUI Properties
**Learning:** Eagerly filtering collections (e.g., `scanResults.filter { ... }.isEmpty`) inside computed properties accessed frequently by SwiftUI causes intermediate array allocations on every read, leading to significant memory churn and potential frame drops.
**Action:** Chain `.lazy.filter` before `.reduce` or `.count` to prevent intermediate allocations, and use `.contains(where:)` to short-circuit boolean existence checks instead of checking `.isEmpty` on a filtered collection.
