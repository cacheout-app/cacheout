...
## 2024-05-24 - SwiftUI Computed Property Memory Churn
**Learning:** SwiftUI computed properties are evaluated on every state change and render pass. Eagerly `.filter`ing arrays inside them (like checking `.isEmpty` on a filtered array or reducing an eagerly filtered array) causes continuous intermediate array allocations, leading to high memory churn and potential stuttering in UI updates.
**Action:** Chain `.lazy.filter` before `.reduce` to prevent intermediate array allocations. For boolean checks, use `.contains(where:)` instead of checking `.isEmpty` on a filtered array to short-circuit evaluation and skip allocations entirely.
