...
## 2025-10-25 - Short-circuit Evaluation in SwiftUI Computed Properties
**Learning:** Using `.isEmpty` on an eagerly filtered array (e.g., `array.filter { condition }.isEmpty`) or reducing an array to check if a value is greater than zero causes unnecessary O(N) evaluation and temporary array allocations. In SwiftUI, these computed properties are accessed frequently during render cycles, causing memory churn.
**Action:** Use `.contains(where: condition)` to allow O(1) early-exit short-circuiting, and `.lazy.filter` before `.reduce` to avoid intermediate array allocations.
