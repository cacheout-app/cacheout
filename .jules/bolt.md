...
## 2026-03-20 - Refactoring filtered array methods into boolean conditions
**Learning:** In Swift, if a method exists purely to filter an array for subsequent iteration (e.g., returning `collection.filter { ... }`), refactoring it into a boolean condition method (e.g., `isCondition(_:) -> Bool`) prevents intermediate array allocations when called within a `for ... where` loop.
**Action:** Refactor methods that return eagerly filtered arrays for iteration into boolean condition checks, and use them directly in `for element in collection where condition` clauses.
