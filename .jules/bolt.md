...
## 2025-11-01 - Eager Filtering in Computed Properties
**Learning:** Calling `.isEmpty` on an eagerly filtered array (e.g., `!selectedResults.isEmpty` where `selectedResults` uses `.filter`) allocates a complete intermediate array just to evaluate a boolean state, causing memory churn in SwiftUI computed properties. Additionally, checking sizes greater than zero (e.g., `selectedNodeModulesSize > 0`) forces full iteration.
**Action:** Use `.contains(where:)` for boolean checks to short-circuit iteration without allocation, and chain `.lazy.filter` before `.reduce`, `.count`, or `.sorted`.
