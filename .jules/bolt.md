...
## 2026-03-20 - Avoid eager filter operations
**Learning:** Eager `.filter` operations on collections create intermediate array allocations that cause unnecessary memory churn, especially in frequently accessed computed properties.
**Action:** Use `.lazy.filter` to avoid array allocations when chaining into `.reduce` or `.count`, and use `.contains(where:)` to check for items rather than evaluating `.isEmpty` on a filtered array.
