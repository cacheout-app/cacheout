...
## 2026-05-31 - Use lazy collections and contains(where:)
**Learning:** Checking `.isEmpty` on a filtered array evaluates the entire array and allocates memory. Similarly, using `.reduce` on an eagerly filtered array creates an intermediate allocation.
**Action:** Use `.contains(where:)` to short-circuit evaluation instead of checking `.isEmpty`, and chain `.lazy.filter` before `.reduce` to prevent intermediate array allocations.
