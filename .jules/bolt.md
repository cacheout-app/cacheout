...
## 2024-05-24 - Avoid Memory Churn in SwiftUI Computed Properties
**Learning:** Computed properties in SwiftUI ViewModels are evaluated frequently. Checking `.isEmpty` on eager `.filter` arrays creates unnecessary intermediate memory allocations. Checking `size > 0` forces full O(N) traversal.
**Action:** Use `.contains(where:)` to short-circuit evaluation and avoid memory allocations. Chain `.lazy.filter` before `.reduce` to eliminate intermediate arrays.
