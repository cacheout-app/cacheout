...
## 2026-04-10 - Avoiding Memory Churn with lazy.filter and contains
**Learning:** Eager `.filter` operations in SwiftUI computed properties create intermediate arrays, leading to memory churn on every UI update. Checking `.isEmpty` on a filtered array is O(N) and allocates memory, whereas `.contains(where:)` is O(1) in the best case and requires no allocation.
**Action:** Chain `.lazy.filter` before `.count` or `.sorted()` to prevent intermediate array allocations. Use `.contains(where:)` to short-circuit evaluation instead of checking `.isEmpty` on filtered collections.
