...
## 2024-05-26 - Accessible Section Headers
**Learning:** In SwiftUI, VoiceOver struggles with complex section header buttons that contain mixed content (icons, text, dynamic counts). If not explicitly configured, it may read them disjointedly or miss the interactive state.
**Action:** When implementing custom section header buttons, apply `.accessibilityElement(children: .combine)` to group the content, add a dynamic `.accessibilityValue` (e.g., 'Expanded' or 'Collapsed'), and an `.accessibilityHint`. Crucially, do not apply an explicit `.accessibilityLabel` to the combined element, as it will override the natural concatenation of the child text values.
