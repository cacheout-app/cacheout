...
**Learning:** Empty states consisting of an icon, a headline, and descriptive text are read as fragmented pieces by VoiceOver, which slows down context comprehension for the user.
**Action:** Apply `.accessibilityElement(children: .combine)` to the container (e.g., `VStack`) wrapping empty state views to ensure VoiceOver announces the entire block as a single unified message.
