...
**Learning:** VoiceOver users experience disjointed and fragmented feedback when empty states are read as separate icon and text elements.
**Action:** Always apply `.accessibilityElement(children: .combine)` to containers wrapping empty state views (typically an icon and text) to ensure VoiceOver announces the empty state as a single unified message.
