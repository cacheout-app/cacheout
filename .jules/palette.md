...
**Learning:** Empty states typically contain an icon, a primary message, and sometimes a secondary explanation. Without grouping, VoiceOver reads these elements separately, creating a disjointed experience for screen reader users.
**Action:** In SwiftUI, apply `.accessibilityElement(children: .combine)` to containers (e.g., `VStack`) wrapping empty state views (typically an icon and text) to ensure VoiceOver announces the empty state as a single unified message.
