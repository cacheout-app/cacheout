...
## 2024-05-24 - Accessibility for Mixed-Content Header Buttons
**Learning:** In SwiftUI, when implementing custom section header buttons with mixed content (e.g., icons, text, dynamic counts), applying an explicit `.accessibilityLabel` overrides the natural concatenation of the child text values.
**Action:** Apply `.accessibilityElement(children: .combine)`, dynamic `.accessibilityValue` ('Expanded' or 'Collapsed'), and an `.accessibilityHint` instead of a static label.
