...
**Learning:** When an HStack or other container groups an icon and descriptive text for a risk warning or alert, VoiceOver reads the elements sequentially as fragmented messages if not combined.
**Action:** Apply `.accessibilityElement(children: .combine)` at the outermost modifier position (after layout modifiers) to prevent VoiceOver from reading the elements sequentially as fragmented messages.
