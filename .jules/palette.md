...
## 2024-05-26 - Custom Section Header Accessibility
**Learning:** In SwiftUI, when implementing custom section header buttons with mixed content (e.g., icons, text, dynamic counts/sizes), applying an explicit `.accessibilityLabel` to the combined element will override and hide the natural concatenation of the child text values from VoiceOver.
**Action:** Apply `.accessibilityElement(children: .combine)`, dynamic `.accessibilityValue` (e.g., 'Expanded' or 'Collapsed'), and an `.accessibilityHint` to ensure VoiceOver correctly reads the full content along with its state, without overriding the label.
