...
## 2024-05-28 - Unified Empty State Announcements
**Learning:** VoiceOver reads empty state text and icons as disjointed fragments, which can make the message harder to understand.
**Action:** Always apply `.accessibilityElement(children: .combine)` to containers (e.g., `VStack`) wrapping empty state views (typically an icon and text) to ensure VoiceOver announces the empty state as a single unified message.
