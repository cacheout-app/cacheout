...
## 2024-05-28 - Unified Empty States
**Learning:** VoiceOver reads empty state text and icons as separate, disjointed elements, causing a fragmented reading experience.
**Action:** Apply `.accessibilityElement(children: .combine)` to containers (e.g., `VStack`) wrapping empty state views (typically an icon and text) to ensure VoiceOver announces the empty state as a single unified message.
