...
## 2024-05-15 - [CRITICAL] Enforce HTTPS for Webhooks
**Vulnerability:** Webhook configuration parsing lacked strict `https` scheme validation, allowing potential plaintext `http` webhooks.
**Learning:** Incomplete validation during config parsing could bypass security checks.
**Prevention:** Ensure strict URL scheme validation (`url.scheme?.lowercased() == "https"`) is applied at the parsing layer.
