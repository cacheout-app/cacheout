...
## 2024-05-29 - TOCTOU Symlink Vulnerability in FileManager Attributes
**Vulnerability:** Calling `FileManager.default.setAttributes` on existing files (like temporary files or sockets) is vulnerable to Time-of-Check to Time-of-Use (TOCTOU) symlink attacks.
**Learning:** An attacker can replace the target file with a symlink right before the attribute modification, causing the application to change permissions on an unintended file. For sockets, `open(2)` fails with `EOPNOTSUPP`, so `fchmod()` cannot be used.
**Prevention:** For regular files, use `open()` with `O_NOFOLLOW | O_CLOEXEC` and apply permissions via `fchmod()`. For sockets, rely entirely on `umask` and strict parent directory permissions, and never use `setAttributes` on the socket file itself.
