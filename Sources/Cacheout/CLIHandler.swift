...
struct CLIHandler {
    ...
    private static func handleClean(slugs: [String], dryRun: Bool) async {
        ...
        let cleanResults: [[String: Any]] = report.cleaned.map { item in
            [
                ...
                "freed_human": byteFormatter.string(fromByteCount: item.bytesFreed),
                "success": true,
            ]
        }
    ...
    private static func handleSmartClean(targetGB: Double, dryRun: Bool) async {
        ...
        for result in sortedResults {
            ...
            if dryRun {
                ...
                cleaned.append([
                    "name": result.category.name,
                    "bytes_freed": freed,
                    "freed_human": byteFormatter.string(fromByteCount: freed),
                ])
            }
        }
        ...
        outputJSON([
            ...
            "total_freed": byteFormatter.string(fromByteCount: freedSoFar),
            "dry_run": dryRun,
            "cleaned": cleaned,
        ] as [String: Any])
    }
        ...
        for result in results where result.exists {
            for url in result.category.resolvedPaths {
                ...
                let markerContent = """
                    ...
                    tagged: \(dateFormatter.string(from: Date()))
                    """
