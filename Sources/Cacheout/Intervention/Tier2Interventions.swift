...
public final class JetsamHWM: Intervention {
    ...
    public func execute(via executor: InterventionExecutor) async -> ExecutionResult {
        ...
        // Optimization: Use .lazy on the sequence to evaluate filter and map lazily
        // directly into the Set, avoiding intermediate array allocations.
    ...
    private func listLocalSnapshots() async throws -> [String] {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            process.terminationHandler = { proc in
                ...
                lock.lock()
                var finalStdout = stdoutData as Data
                var finalStderr = stderrData as Data
                lock.unlock()

                // Read any remaining data in the pipe to ensure completeness
                do {
                    if let remainingOut = try stdoutHandle.readToEnd() {
                        finalStdout.append(remainingOut)
                    }
                    if let remainingErr = try stderrHandle.readToEnd() {
                        finalStderr.append(remainingErr)
                    }
                } catch {
                    // Ignore read errors
                }
