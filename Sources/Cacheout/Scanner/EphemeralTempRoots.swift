...
/// `suppressingAliasShadows`' probe pair (`SpaceScanner.swift:1704-1708`)
...
/// (`suppressingAliasShadows`' doc, `SpaceScanner.swift:1653-1664`), and
...
/// `SpaceScanner.swift:1704-1708`), which every KEPT
/// root reaches.
///
...
///   (`SpaceScanner.swift:1714-1717`) says so:
...
///   `suppressingAliasShadows`' doc (`SpaceScanner.swift:1653-1664`)
...
///   and NO issue channel of its own (`SpaceScanner.swift:1699-1701`; the
///   "bare `[URL]`" this sentence used to say stopped being true when the
///   keys were carried out of the same probe, PR #460 codex r4);
///   `suppressingAliasShadows`' doc (`SpaceScanner.swift:1693-1698`)
///   records what
...
enum EphemeralTempRoots {
    ...
    static func resolve(
        ...
        // (`SpaceScannerRuntime.production` (`SpaceScanner.swift:1841`)), which
        ...
        // (`SpaceScanner.swift:1704-1708`) canonicalizes and probes it —
