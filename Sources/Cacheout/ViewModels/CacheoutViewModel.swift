...
class CacheoutViewModel: ObservableObject {
    ...
    func selectAllSafe() {
        // Optimization: Use .map to batch updates and trigger a single UI notification
        // rather than mutating array elements individually which triggers multiple notifications.
    ...
    func deselectAll() {
        // Optimization: Batch updates to avoid per-element UI refresh notifications
        scanResults = scanResults.map { result in
            var updated = result
