...
struct CleanConfirmationSheet: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("This will remove \(viewModel.formattedTotalSelectedSize) from \(viewModel.selectedResults.count + viewModel.nodeModulesItems.reduce(0) { $0 + ($1.isSelected ? 1 : 0) }) items.")
                .font(.body)
