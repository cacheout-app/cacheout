...
public final class SysctlJournal {
    ...
    private func flushState() -> Bool {
        ...
        do {
            ...
            // Preemptively remove any stale temp file to prevent O_EXCL from failing
            try? FileManager.default.removeItem(at: tmpURL)

            // Securely create temp file with 0600 permissions using POSIX open to prevent TOCTOU
            let fd = tmpURL.withUnsafeFileSystemRepresentation { pathPtr -> Int32 in
                guard let pathPtr = pathPtr else { return -1 }
                return open(pathPtr, O_CREAT | O_WRONLY | O_EXCL | O_CLOEXEC, S_IRUSR | S_IWUSR)
            }
